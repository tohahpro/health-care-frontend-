import { Search, ClipboardList, CalendarCheck, ShieldCheck, FileText, Video, CreditCard, HeartPulse, Boxes } from 'lucide-react';
import React from 'react';
import { Card, CardContent } from '@/components/ui/card';

const steps = [
  { icon: Search, title: 'Search Doctor', description: 'Find your doctor easily with a minimum of effort.' },
  { icon: Boxes, title: 'AI Precision Matching', description: 'Tell our system your symptoms or the type of specialist you need.' },
...
const StepCard = ({ icon: Icon, title, description, index }: { icon: React.ElementType, title: string, description: string, index: number }) => {
    const bgColors = [
        'bg-blue-50', 'bg-red-50', 'bg-pink-50', 'bg-green-50', 'bg-yellow-50',
        'bg-pink-50', 'bg-blue-50', 'bg-yellow-50', 'bg-green-50'
    ];
    const textColors = [
        'text-blue-500','text-red-500', 'text-pink-500', 'text-green-500', 'text-yellow-500',
